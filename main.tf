...
resource "aws_iam_role_policy" "exporter_kinesis" {
  ...
  policy = jsonencode({
    Version   = "2012-10-17",
    Statement = [{ Effect = "Allow", Action = ["kinesis:PutRecord","kinesis:PutRecords"], Resource = aws_kinesis_stream.telemetry_stream.arn }]
  })
}
...
EOF
  depends_on = [aws_cloudwatch_log_group.springboot_logs, aws_cloudwatch_log_group.otel_collector_logs]
  volume { name = "otel-volume" }
}
...
EOF
  depends_on = [aws_cloudwatch_log_group.otel_importer_logs]
}
